...
thresh_max = max(ID_cam_gallery(:, 3));
thresh_min = min(ID_cam_gallery(:, 3));
inter = (thresh_max - thresh_min)/30;
thresh = [thresh_min:inter:thresh_max];
...
map = zeros(length(thresh), 1);
cmc_avg = cell(length(thresh), 1);
r1 = zeros(length(thresh), 1); 
r20 = zeros(length(thresh), 1); 
nwindow = zeros(length(thresh), 1);
for i = 1:length(thresh)
    thre = thresh(i);
    pos2 = find(ID_cam_gallery(:, 3) >= thre);
